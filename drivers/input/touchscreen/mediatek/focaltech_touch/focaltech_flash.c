...
static int fts_ecc_check(const u8 *buf, u32 len, u32 ecc_saddr) {
  ...
  for (i = 0; i < packet_number; i++) {
    ...
    if (ecc_in_tp != ecc_in_host) {
      FTS_ERROR("ecc_in_tp(%x) != ecc_in_host(%x), ecc check fail", ecc_in_tp,
                ecc_in_host);
      return -EIO;
    }
...
static int fts_pram_write_ecc(const u8 *buf, u32 len) {
  ...
  /* get pram app length */
  code_len =
      ((u16)buf[FTS_APP_INFO_OFFSET + 0] << 8) + buf[FTS_APP_INFO_OFFSET + 1];
  code_len_n =
      ((u16)buf[FTS_APP_INFO_OFFSET + 2] << 8) + buf[FTS_APP_INFO_OFFSET + 3];
 ...
 * FTS_- MODULE_ID/FTS_MODULE_NAME; If get fw via .i file, please set
 * FTS_FW_REQUEST_SUPPORT=0, and F- TS_MODULE_ID; will use module id to
 * distingwish different modules; If get fw via reques_firmware(), please set
 * FTS_FW_REQUEST_SUPPORT =1, and FTS_MODULE_NAME; fw file name will be composed
 * of "focalt- ech_ts_fw_" & FTS_VENDOR_NAME;
 *
  ...
  if (1 == setting_count) {
    ...
    for (i = 0; i < setting_count; i++) {
      ...
      if ((setting->rom_idh == ts_data->ic_info.ids.rom_idh) &&
        ...
        FTS_INFO("match upgrade setting,type(ID):0x%02x%02x", setting->rom_idh,
                 setting->rom_idl);
        fwupgrade->setting_nf = setting;
      }
    }
