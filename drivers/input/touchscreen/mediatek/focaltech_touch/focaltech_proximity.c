 ...
 * Revision History:
 *		v1.0:
 *			First release based on xiaguobin's solution. By
 * luougojin 2016-08-19
 *****************************************************************************/
 ...
 * data_path FTS_ALSPS_SUPPORT = 1, is control_path, data_path FTS_ALSPS_SUPPORT
 * = 0, hwmsen_object
 */
...
static int ps_set_delay(u64 ns) { return 0; }
...
static int ps_flush(void) { return 0; }
#endif
...
int ps_local_uninit(void) { return 0; }
...
int fts_proximity_readdata(struct fts_ts_data *ts_data) {
  ...
  if (proximity_status != (int)fts_proximity_data.detect) {
    ...
    if (ret) {
      FTS_ERROR("[PROXIMITY] Call hwmsen_get_interrupt_data failed, ret=%d",
                ret);
      return ret;
    }
#endif
...
int fts_proximity_exit(void) { return 0; }
